...
# 新增: Refresh Token 的過期時間 (通常設比較長，例如 7 天)
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), response: Response = None):
    ...
    # 2. 建立 Refresh Token (通常只包含必要的識別資訊)
    ...
    # 設定 Refresh Token Cookie (這裡設為 httponly 確保安全性，且路徑設為 /refresh 避免隨處傳送)
    response.set_cookie(
        key="refresh_token",
        value=refresh_token,
        httponly=True,
        samesite="lax",
        # path="/refresh" # 在生產環境建議限制 path，測試時可先註解
    )
...
# 新增: Refresh Token 功能
@app.post("/refresh")
def refresh_endpoint(response: Response, refresh_token: Optional[str] = Cookie(None)):
    """
    接收 cookie 中的 refresh_token，驗證後回傳新的 access_token
    """
    ...
    try:
        ...
        # (選擇性) 這裡通常會檢查資料庫，確認使用者是否被停權，或 Refresh Token 是否被撤銷 (Blacklist)
